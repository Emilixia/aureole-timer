...
const Animations = (function () {
  ...
  function _initBgState(canvas) {
    ...
    if (mode === 'fireflies') {
      ...
      for (var f = 0; f < 60; f++) {
        _bgState.flies.push({
          ...
          wobble:Math.random() * 0.04 + 0.01,
          phase: Math.random() * Math.PI * 2,
          alpha: Math.random() * 0.4 + 0.2,
          aDelta:(Math.random() - 0.5) * 0.012
        });
      }
    }
    ...
    if (mode === 'neon-grid') {
      _bgState.t   = 0;
      _bgState.cols = Math.floor(W / 60) + 1;
      _bgState.rows = Math.floor(H / 60) + 1;
      _bgState.pulses = [];
      for (var p = 0; p < 6; p++) {
        _bgState.pulses.push({
          x: Math.floor(Math.random() * _bgState.cols),
          y: Math.floor(Math.random() * _bgState.rows),
          life: Math.random() * 80,
  ...
  function _renderBgEffect(canvas, ctx, mode, st) {
    ...
    if (mode === 'fireflies') {
      ctx.clearRect(0, 0, W, H);
      for (var f = 0; f < st.flies.length; f++) {
        var fl = st.flies[f];
        fl.phase  += fl.wobble;
        fl.angle  += (Math.random() - 0.5) * 0.08;
        fl.x      += Math.cos(fl.angle) * fl.speed;
        fl.y      += Math.sin(fl.angle) * fl.speed + Math.sin(fl.phase) * 0.4;
        fl.alpha  += fl.aDelta;
        if (fl.alpha <= 0.08) fl.aDelta =  Math.abs(fl.aDelta);
        if (fl.alpha >= 0.8)  fl.aDelta = -Math.abs(fl.aDelta);
        if (fl.x < -20)  fl.x = W + 20;
        if (fl.x > W+20) fl.x = -20;
        if (fl.y < -20)  fl.y = H + 20;
        if (fl.y > H+20) fl.y = -20;
        var grd = ctx.createRadialGradient(fl.x, fl.y, 0, fl.x, fl.y, fl.r * 4);
        grd.addColorStop(0, 'rgba(170,255,100,' + fl.alpha + ')');
        grd.addColorStop(0.4, 'rgba(120,230,60,' + (fl.alpha * 0.5) + ')');
        grd.addColorStop(1, 'rgba(80,180,30,0)');
        ctx.beginPath();
        ctx.arc(fl.x, fl.y, fl.r * 4, 0, Math.PI * 2);
        ctx.fillStyle = grd;
        ctx.fill();
        ctx.beginPath();
        ctx.arc(fl.x, fl.y, fl.r * 0.7, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(230,255,180,' + Math.min(1, fl.alpha * 1.6) + ')';
        ctx.fill();
      }
    }
    ...
    if (mode === 'snow') {
      ctx.clearRect(0, 0, W, H);
      for (var s = 0; s < st.flakes.length; s++) {
        var fl2 = st.flakes[s];
        fl2.phase += fl2.wobble;
        fl2.y     += fl2.speed;
        fl2.x     += fl2.drift + Math.sin(fl2.phase) * 0.3;
        if (fl2.y > H + 10) { fl2.y = -10; fl2.x = Math.random() * W; }
        if (fl2.x < -10)    fl2.x = W + 10;
        if (fl2.x > W + 10) fl2.x = -10;
        ctx.beginPath();
        ctx.arc(fl2.x, fl2.y, fl2.r, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(210,230,255,' + fl2.alpha + ')';
        ctx.fill();
        if (fl2.r > 1.5) {
          ctx.strokeStyle = 'rgba(180,210,255,' + (fl2.alpha * 0.4) + ')';
          ctx.lineWidth = 0.5;
          var arm = fl2.r * 2.2;
          for (var a = 0; a < 3; a++) {
            var ang = (a / 3) * Math.PI;
            ctx.beginPath();
            ctx.moveTo(fl2.x - Math.cos(ang) * arm, fl2.y - Math.sin(ang) * arm);
            ctx.lineTo(fl2.x + Math.cos(ang) * arm, fl2.y + Math.sin(ang) * arm);
            ctx.stroke();
          }
        }
      }
    ...
    if (mode === 'neon-grid') {
      ...
      ctx.lineWidth = 0.5;
      // Vertical lines
      for (var gc = 0; gc <= st.cols; gc++) {
        var gx = gc * CZ;
        ctx.strokeStyle = 'rgba(100,60,200,0.12)';
        ctx.beginPath(); ctx.moveTo(gx, 0); ctx.lineTo(gx, H); ctx.stroke();
      }
      // Horizontal lines
      for (var gr = 0; gr <= st.rows; gr++) {
        var gy = gr * CZ;
        ctx.strokeStyle = 'rgba(100,60,200,0.12)';
        ctx.beginPath(); ctx.moveTo(0, gy); ctx.lineTo(W, gy); ctx.stroke();
      }
      // Intersection dots and pulse rings
      for (var pi = 0; pi < st.pulses.length; pi++) {
        var pulse = st.pulses[pi];
        pulse.life++;
        if (pulse.life >= pulse.maxLife) {
          pulse.x = Math.floor(Math.random() * st.cols);
          pulse.y = Math.floor(Math.random() * st.rows);
          pulse.life = 0;
          pulse.maxLife = 80 + Math.random() * 80;
        }
        var prog = pulse.life / pulse.maxLife;
        var pr   = prog * CZ * 2.5;
        var pa   = (1 - prog) * 0.55;
        ctx.beginPath();
        ctx.arc(pulse.x * CZ, pulse.y * CZ, pr, 0, Math.PI * 2);
        ctx.strokeStyle = 'rgba(160,80,255,' + pa + ')';
        ctx.lineWidth = 1.2;
        ctx.stroke();
      }
      // Intersection dots
      for (var ic = 0; ic <= st.cols; ic++) {
        for (var ir = 0; ir <= st.rows; ir++) {
          var pha = Math.sin(st.t * 0.025 + ic * 0.5 + ir * 0.7) * 0.5 + 0.5;
          ctx.beginPath();
          ctx.arc(ic * CZ, ir * CZ, 1.2, 0, Math.PI * 2);
          ctx.fillStyle = 'rgba(180,100,255,' + (0.15 + pha * 0.35) + ')';
          ctx.fill();
        }
      }
    }
